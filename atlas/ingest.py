...
_META_DDL = (
    ...
    # One row per message id whose classification is *final* — see
    # `note_settled()`. This is the second half of the incremental scan: the
    # cursor stops the walk from descending past what it has already seen, and
    # this table stops it re-downloading the documents it has already resolved
    # in the range it does walk.
    #
    # Both are needed. The cursor cannot advance across a gap (a scan capped by
    # `max_messages`, or one that died mid-walk), and in exactly those cases the
    # next walk re-covers ground. Without this table that re-walk re-downloads
    # every bundle manifest and every asset manifest it meets, which on a
    # channel of a few thousand messages is minutes of Telegram traffic to learn
    # nothing.
    "CREATE TABLE IF NOT EXISTS scan_seen "
    "(msg_id INTEGER PRIMARY KEY, verdict TEXT, at REAL)",
)
...
def scan_cursor(conn: sqlite3.Connection) -> int:
    """The highest message id a *complete* walk has already covered.

    Zero means no complete walk has ever finished, which is the only state in
    which a walk down to message 1 is the right thing to do.
    """
    try:
        return int(meta_get(conn, "last_scan_head", 0) or 0)
    except (TypeError, ValueError):
        return 0


def scan_floor(conn: sqlite3.Connection, head: int, max_messages: int = 0,
               full_rescan: bool = False) -> tuple:
    """Where the backwards walk stops, and whether the cursor may advance.

    Upstream this was `floor = 1`, unconditionally, and the cursor written at
    the end of every scan was never read back — so every relaunch re-walked the
    entire channel from the newest message down to id 1 to rediscover thousands
    of messages it had already classified. Reading it back is the whole fix.

    The returned `advance` flag is the part that is easy to get wrong. The
    cursor is a claim that *everything* below it has been covered, so it may
    only move when this walk actually joins up with the last one. Two cases
    break that join:

      * `max_messages` caps the walk to the newest N ids, leaving a gap between
        the old cursor and this floor;
      * the walk dies partway, in which case the caller never gets here at all.

    Advancing across a gap would make the skipped range permanently invisible —
    a silent, unrecoverable hole in the archive, and precisely the failure the
    `full_rescan` override exists to repair. So a capped scan reports
    `advance=False` and the cursor stays where it was.
    """
    cursor = 0 if full_rescan else scan_cursor(conn)
    base = cursor + 1 if cursor else 1
    floor = base
    if max_messages:
        floor = max(base, head - int(max_messages) + 1)
    floor = max(1, min(floor, head))
    return floor, floor <= base, cursor


def settled_ids(conn: sqlite3.Connection, low: int, high: int) -> set:
    """Message ids in [low, high] whose classification cannot change."""
    try:
        return {r[0] for r in conn.execute(
            "SELECT msg_id FROM scan_seen WHERE msg_id BETWEEN ? AND ?",
            (int(low), int(high)))}
    except sqlite3.Error:
        return set()


def note_settled(conn: sqlite3.Connection, rows: list) -> None:
    """Record `[(msg_id, verdict), …]` as resolved for good.

    Only outcomes a re-walk could not improve on belong here: a bundle, shard
    or asset set that imported cleanly, and a message carrying nothing this
    reader wants. **Failures never settle** — a manifest that would not parse is
    ...
    try:
        conn.executemany(
            "INSERT OR REPLACE INTO scan_seen(msg_id, verdict, at) "
            "VALUES (?,?,?)",
            [(int(m), str(v), time.time()) for m, v in rows])
        conn.commit()
    except sqlite3.Error as e:
        # A lost skip record costs one redundant download later. It must never
        # cost the scan.
        log(f"could not record {len(rows)} scan verdict(s) ({e})")
...
def scan_and_import(full: bool = True, max_messages: int = 0,
                    on_bundle=None, full_rescan: bool = False) -> dict:
    ...
    Two phases, in this order on purpose:
      ...
      2. The backwards walk, if `full`. Batches of ids from the newest down to
         the scan cursor, looking for manifests.
    ...
    **The walk is incremental here, which it was not upstream.** Phase 2 stops
    at `last_scan_head + 1` rather than at message id 1, so a relaunch examines
    what arrived since the last complete walk instead of re-examining the whole
    channel. `full_rescan=True` forces the walk back to id 1 — the repair for a
    cursor that advanced over a range it should not have, and the honest thing
    to offer in the UI next to "rescan channel".
    """
    if _STATE["running"]:
    ...
    try:
        ...
        floor, may_advance, cursor = scan_floor(conn, head, max_messages,
                                                full_rescan)
        span = head - floor + 1
        if cursor and not full_rescan:
            log(f"incremental scan — last complete walk reached {cursor}, "
                f"so {span} new message id(s) to examine, not {head}")
        settled = settled_ids(conn, floor, head)
        if settled:
            log(f"{len(settled)} message id(s) in range already resolved — "
                f"skipping their downloads")
        _set(phase="scanning", scan_total=span,
             detail=f"walking messages {head} → {floor}")
        log(f"scanning {span} message ids for manifests")

        BATCH = 190          # get_messages tolerates 200; leave headroom
        mid = head
        while mid >= floor:
            ids = [i for i in range(max(floor, mid - BATCH + 1), mid + 1)
                   if i not in settled]
            verdicts = []
            if not ids:
                # Every id in this batch was resolved by an earlier walk, so
                # there is nothing to fetch. Still count them as scanned, or the
                # progress bar stalls on a channel that is mostly already known.
                with _LOCK:
                    _STATE["scanned"] += min(BATCH, mid - floor + 1)
                mid -= BATCH
                continue
            try:
            ...
            for msg in reversed(msgs or []):
                info = tgchannel.message_document(msg)
                if not info:
                    # No document at all — a text post, a photo, a service
                    # message. Nothing here will ever become interesting.
                    m_id = getattr(msg, "id", 0) or 0
                    if m_id:
                        verdicts.append((m_id, "no-document"))
                    continue
                if info.get("message_id") in found_ids:
                    continue
                ...
                if not man:
                    ...
                    if _asset_manifest(conn, info, work):
                        found_ids.append(m_id)
                        verdicts.append((m_id, "asset"))
                        with _LOCK:
                            _STATE["assets"] = _STATE.get("assets", 0) + 1
                    elif not _looks_like_asset_manifest(info) and \
                            not tgchannel.looks_like_manifest(info):
                        # Neither shape. A plain video, an info json, a poster —
                        # real channel content that this reader has no interest
                        # in, and never will. Settling it is what stops a
                        # re-walk paying the name checks and the log line again.
                        verdicts.append((m_id, "uninteresting"))
                    continue
                found_ids.append(m_id)
                ...
                if seq in seen:
                    _set(skipped=_STATE["skipped"] + 1)
                    # The manifest download that got us here is the cost this
                    # record exists to avoid next time.
                    verdicts.append((m_id, "bundle"))
                    continue
                _set(phase="importing")
            ...
            # `get_messages` drops ids that came back empty — deleted messages
            # and service events leave gaps all through a channel's history, and
            # a deleted id will never come back. Settle them too, but *only* if
            # the call itself succeeded: on a network failure the whole batch is
            # absent and settling it would silently erase 190 real messages from
            # every future walk.
            if fetched:
                got = {getattr(m, "id", 0) for m in (msgs or [])}
                verdicts.extend((i, "absent") for i in ids if i not in got)

            # One commit per batch rather than per message: a few thousand
            # single-row commits on WAL is seconds of fsync for bookkeeping.
            note_settled(conn, verdicts)
            mid -= BATCH
        ...
        if may_advance:
            ...
            log(f"cursor left at {cursor} — this walk stopped at {floor} and "
                f"did not cover everything below it")
        _set(phase="done", running=False, finished_at=time.time(),
...
def scan_in_background(full: bool = True, max_messages: int = 0,
                       on_bundle=None, full_rescan: bool = False) -> bool:
    if _STATE["running"]:
        return False
    t = threading.Thread(target=scan_and_import,
                         args=(full, max_messages, on_bundle, full_rescan),
