...
def rescan(full: bool = True, max_messages: int = 0,
           full_rescan: bool = False) -> bool:
    ...
    return ingest.scan_in_background(full=full, max_messages=max_messages,
                                     on_bundle=after, full_rescan=full_rescan)
...
def api_scan(full: bool = True, max_messages: int = 0,
    ...
    The default walk is incremental: it examines what has arrived since the last
    complete walk. `full_rescan=true` walks back to message id 1 — slow, and the
    right button to offer when the cursor is suspected of having advanced over
    something.
    """
